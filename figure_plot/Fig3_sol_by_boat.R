...

{
  ...
  for (plot_random_sample in c(F, T)) {
    for (istrata in stratas) {
      ...
      for(isample in samples){
        ...
        if(istrata != 15) {
          sol_col = brewer.pal(n = istrata, 
                               name = "Paired")
        }
        
        if(istrata == 15) {
          sol_col = c(brewer.pal(n = 12, 
                                 name = "Paired"),
                      rev(brewer.pal(n = 11, 
        ...
        text(x = goa_ras@extent[1] + xrange * 0.70, 
             y = goa_ras@extent[3] + yrange * 0.55,
             labels = paste0("n = ", isample, "\n"))
        
        #If Strata == 5, plot subtitle for Simulated Survey
       if (istrata == 5)  mtext(side = 3, 
                                text = ifelse(plot_random_sample, 
                                              "Strata with Simulated Survey",
                                              "Strata Only"), 
                                line = 2,
                                font = 2)
